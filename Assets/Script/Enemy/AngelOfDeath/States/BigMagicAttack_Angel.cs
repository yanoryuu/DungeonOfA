...
public class BigMagicAttack_Angel : IAngelState
{
     // 移動離脱の設定
    ...
    {
        Debug.Log("Enter BigMagic");

        _isMovingAway.Where(x => x == false)
            .Subscribe(_ =>
            {
                AnimatorCommon.SetAction("BigMagicAttack", _enemy.Animator);

                Observable.Interval(TimeSpan.FromSeconds(1))
                    .Take(1)
                    .Subscribe(_ =>
                    {
                        _enemy.AngelAttackManager.MagicAttack3(_enemy);
    ...
    {
        AnimatorCommon.ResetAction("BigMagicAttack", _enemy.Animator);
        if (_enemy.NavMeshAgent != null)
        {
